...
namespace BankApplication
{
    class Program
    {
        ...
        {
            Bank<Account> bank = new Bank<Account>("Приорбанк");
            bool alive = true;
            while (alive)
            {
                ConsoleColor color = Console.ForegroundColor;
                Console.ForegroundColor = ConsoleColor.DarkMagenta; //вывод списка команд
        ...
        private static void WithdrawSumHandler(object sender, AccountEventArgs e)
        {
            Console.WriteLine(e.Message);
            if (e.Sum > 0)
                Console.WriteLine("Время тратить деньги!!!");
        }
